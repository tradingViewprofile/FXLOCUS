...
async function notifyAdmins(admin: ReturnType<typeof supabaseAdmin>, user: { id: string; email?: string | null; full_name?: string | null; leader_id?: string | null }) {
  ...
  await admin.from("notifications").insert(
    Array.from(targets).map((id) => ({
      ...
      title: "交易日志提交 / Trade log submitted",
      content: `学员 ${label} 已提交交易日志。\n\nStudent ${label} submitted trade logs.`
    })) as any
  );
}
...
export async function POST(req: Request) {
  try {
    ...
    const submissionId = replaceId || randomUUID();
    let oldFiles: Array<{ id: string; storage_bucket: string; storage_path: string }> = [];
    ...
    try {
      ...
      if (newPaths.length) {
        await admin.from("trade_submission_files").delete().eq("submission_id", submissionId).in("storage_path", newPaths);
      }
      if (!isReplace) {
